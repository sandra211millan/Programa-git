...
function Agenda () {
	...
	this.Contacto = function(nombre,numero,correo,tratamiento){
		this.nombre = nombre;
		this.numero = numero;
		this.correo = correo;
	...
	this.Insertar = function(){
        ...
        var trat= prompt('Tratamiento');
		if (nom != ""  && num != "") 
		{
			var c = new this.Contacto(nom,num,cor,trat);
			memoria.push(c);
	...
	this.Guardar = function(arr)
	{
		for (var i = arr.length-1; i < arr.length; i++) {
			...
			{				
				...
				var info = d.createTextNode('Nombre: '+ arr[i].nombre + ' | Telefono: '+ arr[i].numero + ' | Correo: '+ arr[i].correo  + ' | Tratamiento: ' + arr[i].tratamiento );
				li.appendChild(info);
				ul.appendChild(li);
			}
			else
